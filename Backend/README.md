...
### Utilidades de EscuelaIT

**Payload para todos los endpoints de EscuelaIT:**

```json
{
  "url": "https://escuela.it/cursos/..."
}
```

- `POST /api/is-valid-escuelait-url`: Valida si una URL corresponde a una clase de EscuelaIT.
- `POST /api/get-escuelait-video-details`: **(Recomendado)** Obtiene todos los detalles (título, duración y miniatura) en una sola llamada.
- `POST /api/get-escuelait-title`: Obtiene solo el título del video.
- `POST /api/get-escuelait-duration`: Obtiene solo la duración del video.
- `POST /api/get-escuelait-thumbnail`: Obtiene solo la URL de la miniatura del video.
- `POST /api/get-escuelait-transcript-plain-text`: Obtiene la transcripción en texto plano.
- `POST /api/get-escuelait-transcript-vtt`: Obtiene la transcripción en formato VTT.
...
**Payload para todos los endpoints de YouTube:**
...
- `POST /api/get-youtube-video-details`: **(Recomendado)** Obtiene todos los detalles (título, duración y miniatura) en una sola llamada.
- `POST /api/get-youtube-thumbnail`: Obtiene la URL de la miniatura del video.
...
### Agente (Procesamiento con IA)

- `POST /agent/process-vtt-escuelait`: Procesa una URL de EscuelaIT, extrae el VTT y genera un resumen en Markdown.
  - **Body:** `{ "url": "https://escuela.it/..." }`
- `POST /agent/process-vtt-youtube`: Procesa una URL de YouTube, extrae el VTT y genera un resumen en Markdown.
